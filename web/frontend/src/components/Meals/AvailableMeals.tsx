...
import MeatItem from "../Meals/MealItem/MealItem";
...
const AvailableMeals = () => {
  const [meals, setMeals] = useState<Array<mealItemProps>>([]);
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [httpError, setHttpError] = useState<any>(null);

  useEffect(() => {
    const fetchMeals = async () => {
      const response = await fetch("https://food-delivery-a9baf-default-rtdb.firebaseio.com/meals.json");
      if (!response.ok) {
        throw new Error("Something went wrong!");
      }
      const responseData = await response.json();
      const loadedMeals = new Array<mealItemProps>();
